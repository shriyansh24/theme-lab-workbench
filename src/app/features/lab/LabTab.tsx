...
export function LabTab({
  ...
  const updateColor = (token: keyof ThemeColors, value: string) => {
    setColors(prev => ({ ...prev, [token]: value }));
  };

  const resetToken = (token: keyof ThemeColors) => {
    setColors(prev => ({ ...prev, [token]: initialTheme.colors[token] }));
  };

  const resetAll = () => {
    setColors(initialTheme.colors);
    setRadius(initialTheme.radius);
  ...
  const randomizeTheme = () => {
    ...
    // Keep semantic/charts mostly similar or slightly adjusted
    next.suc = chroma.hsl(140, 0.7, 0.4).hex();
    next.warn = chroma.hsl(40, 0.9, 0.5).hex();
    next.dan = chroma.hsl(0, 0.8, 0.5).hex();
    
    // Keep charts as is from current state to avoid huge disruption, or randomize hues
    for (let i = 1; i <= 6; i++) {
      const chKey = `ch${i}` as keyof ThemeColors;
      if (!lockedTokens.has(chKey)) {
        next[chKey] = chroma.hsl((baseHue + i * 60) % 360, 0.7, 0.6).hex();
      }
    }
  ...
  const invertTheme = () => {
    setColors(prev => {
      ...
      for (const k in next) {
        ...
        if (!lockedTokens.has(key)) {
          try {
            const c = chroma(prev[key]);
            const [h, s, l] = c.hsl();
            // Flip lightness, keep hue/sat
            next[key] = chroma.hsl(h, s, 1 - l).hex();
          } catch {
            // keep if error (e.g. shadow rgba)
          }
        }
      }
      // Fix up specific text contrasts to ensure readability
  ...
  const generateFromPrimary = () => {
    try {
      ...
      // Monochromatic backgrounds based on primary
      const bgSat = Math.min(s, 0.15); // subtle tint
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Simple average color (can be improved with clustering later)
        ...
        // Use this as the base for a new theme
        const isDark = Math.random() > 0.5; // Randomly choose light/dark or base it on image luminance
        const next: Partial<ThemeColors> = {};
        ...
        next.text = chroma.hsl(h, bgSat, isDark ? 0.95 : 0.10).hex();
        // Set just a few to show the idea
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, padding: 12, background: '#18181b', border: '1px solid #27272a', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, paddingLeft: 16, borderLeft: '1px solid #3f3f46', height: 40, alignItems: 'center' }}>
          <button
            ...
            title="Randomize (I'm Feeling Lucky)"
            style={{ background: '#27272a', color: '#fafafa', border: '1px solid #3f3f46', padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}
          >
          ...
          <button
            ...
            title="Generate scheme from Primary color"
            style={{ background: '#27272a', color: '#fafafa', border: '1px solid #3f3f46', padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}
          >
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, color: '#71717a', alignSelf: 'center' }}>
        ...
        <div style={{ border: '1px solid #27272a', background: '#09090b', padding: 16, maxHeight: 800, overflowY: 'auto' }}>
          ...
          {Object.entries(TOKEN_GROUPS).map(([groupName, tokens]) => (
            <div key={groupName} style={{ marginBottom: 24 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#a1a1aa', marginBottom: 8, borderBottom: '1px solid #27272a', paddingBottom: 4 }}>
                {groupName}
              </div>
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <input
                        ...
                        style={{
                          ...mono,
                          flex: 1,
                          background: isLocked ? '#18181b' : '#09090b',
                          color: isLocked ? '#52525b' : '#fafafa',
                          border: '1px solid #3f3f46',
                          padding: '4px 6px',
                          fontSize: 11
                        }}
