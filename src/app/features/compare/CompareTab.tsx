...
export function CompareTab({ allThemes, themeNames, screen, onScreenChange, screens, families }: CompareTabProps) {
  ...
  const quickCompare = (family: string, mode: 'light' | 'dark') => {
    ...
    if (cur && pro) { setCompareA(cur); setCompareB(pro); }
  };
    ...
    <div>
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          {families.map(f => (
            <button key={f} onClick={() => quickCompare(f, 'dark')}
              style={{ padding: '4px 8px', fontSize: 10, background: '#27272a', color: '#a1a1aa', border: '1px solid #3f3f46', cursor: 'pointer', fontFamily: 'inherit' }}>
              {f} (dark)
            </button>
          ))}
        </div>
          ...
          <div key={name} style={{ border: '2px solid #27272a', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '8px 12px', background: '#111', display: 'flex', gap: 3, flexWrap: 'wrap' }}>
              {(['bg','bgSec','bgTer','bgEl','text','textSec','textMut','pri','suc','warn','dan','border','ch1','ch2','ch3','ch4','ch5','ch6'] as (keyof ThemeColors)[]).map(k => (
                <div key={k} title={`${k}: ${theme.colors[k]}`} style={{ width: 18, height: 18, background: theme.colors[k], border: '1px solid #27272a' }} />
              ))}
            </div>
          </div>
        ))}
      ...
      <div style={{ background: '#09090b', border: '1px solid #27272a', padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#a1a1aa', cursor: 'pointer' }}>
            <input type="checkbox" checked={showOnlyDifferent} onChange={e => setShowOnlyDifferent(e.target.checked)} style={{ cursor: 'pointer' }} />
            Show only different
          </label>
        </div>
              ...
              {displayedDiffs.map(({ token, valueA, valueB, changed }) => (
                <tr key={token} style={{ borderBottom: '1px solid #1a1a1a' }}>
                  ...
                  <td style={{ textAlign: 'center', padding: 8 }}>
                    {changed ? (
                      <span style={{ ...mono, fontSize: 9, padding: '2px 6px', background: '#b45309', color: '#fbbf24', fontWeight: 600 }}>CHANGED</span>
                    ) : (
                      <span style={{ ...mono, fontSize: 9, padding: '2px 6px', background: '#14532d', color: '#86efac' }}>SAME</span>
                    )}
                  </td>
                </tr>
              ))}
        ...
        {showOnlyDifferent && displayedDiffs.length === 0 && (
          <div style={{ textAlign: 'center', padding: 32, color: '#52525b', fontSize: 13 }}>No differences found between these themes</div>
        )}
      </div>
    </div>
  );
