...
const shortName = (n: string) =>
  n.replace('[Current] ', 'C:').replace('[Proposed] ', 'P:').replace('[Shadcn] ', 'S:').replace('[Refero] ', 'R:').replace('[Research] ', 'X:');
  ...
  return (
    <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #27272a' }}>
              <th style={{ textAlign: 'left', padding: 8, color: '#71717a', position: 'sticky', left: 0, background: '#09090b', zIndex: 2, minWidth: 140 }}>Check</th>
              <th style={{ textAlign: 'center', padding: 8, color: '#71717a', position: 'sticky', left: 140, background: '#09090b', zIndex: 2, minWidth: 70 }}>Usage</th>
              {themeNames.map(n => (
                <th key={n} style={{ padding: '8px 4px', color: '#52525b', fontSize: 8, textAlign: 'center', minWidth: 60 }}>{shortName(n)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {checks.map(({ label, fg, bg, usage, note }) => (
              <tr key={label} style={{ borderBottom: '1px solid #1a1a1a' }} title={note}>
                <td style={{ ...mono, padding: 8, color: '#a1a1aa', position: 'sticky', left: 0, background: '#09090b', zIndex: 1 }}>{label}</td>
                <td style={{ textAlign: 'center', padding: 4, position: 'sticky', left: 140, background: '#09090b', zIndex: 1 }}>
                  <span style={{ ...mono, fontSize: 8, padding: '1px 5px', background: usageBg[usage], color: usageColor[usage] }}>{usageLabel[usage]}</span>
                </td>
                {themeNames.map(n => {
                  ...
                  const badgeBg = passes ? (ratio >= 7 ? '#065f46' : ratio >= 4.5 ? '#1e3a5f' : '#14532d') : '#7f1d1d';
                  const badgeColor = passes ? (ratio >= 7 ? '#6ee7b7' : ratio >= 4.5 ? '#7dd3fc' : '#86efac') : '#fca5a5';
                  return (
                    <td key={n} style={{ textAlign: 'center', padding: 4 }}>
                      <span style={{ ...mono, fontSize: 9, padding: '1px 5px', background: badgeBg, color: badgeColor, fontWeight: 600 }}>
      ...
      <div style={{ marginTop: 24, background: '#111', border: '2px solid #27272a', padding: 16 }}>
        <div style={{ ...display, fontSize: 14, fontWeight: 700, color: '#fafafa', marginBottom: 12 }}>Token Usage Map</div>
        <div style={{ fontSize: 12, color: '#71717a', marginBottom: 12 }}>How each color token is actually used in the UI \u2014 determines whether a contrast failure is a real bug or acceptable.</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[
            ...
            { token: 'textMut', where: 'Timestamps, placeholders, axis labels, salary ranges', verdict: 'Decorative \u2014 no hard minimum, but \u22653:1 is nice' },
            { token: 'pri', where: 'Stat numbers (large), links (small), button bg', verdict: '\u22653:1 as large text/icon, \u22654.5:1 as small text' },
            { token: 'suc / warn / dan', where: 'Badge fills (text is white/black via textOn), toast accents', verdict: 'Contrast matters for textOn(fill), not fill-on-bg' },
            { token: 'sideATxt / sideABg', where: 'Active sidebar nav item', verdict: 'Must pass AA (4.5:1) \u2014 this is readable text' },
            { token: 'ch1\u2013ch6', where: 'Chart bars, pie segments, legend dots', verdict: 'Decorative / adjacent-color distinguishability' },
            { token: 'border / shadow', where: 'Card borders, hard-offset shadows', verdict: 'Visual separation only, no text contrast needed' }
          ].map(row => (
            <div key={row.token} style={{ background: '#18181b', padding: 10, borderLeft: '3px solid #a78bfa' }}>
              <div style={{ ...mono, fontSize: 11, fontWeight: 700, color: '#a78bfa', marginBottom: 4 }}>{row.token}</div>
