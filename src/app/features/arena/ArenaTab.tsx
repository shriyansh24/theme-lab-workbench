...
export function ArenaTab({
  ...
  const arenaThemes = useMemo(() => {
    return themeNames.filter(n => allThemes[n].family === arenaFamily);
  }, [themeNames, allThemes, arenaFamily]);

  const ranking = useMemo(() =>
    [...themeNames].sort((a, b) => scores[b] - scores[a]).filter(n => scores[n] > 0),
    [themeNames, scores]
  );
          ...
          {ranking.map((name, i) => {
            ...
            return (
              <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, padding: '4px 0' }}>
                <span style={{ ...mono, fontSize: 14, fontWeight: 800, color: i === 0 ? '#fbbf24' : '#52525b', width: 20 }}>
                  {i + 1}
                </span>
                ...
                <div style={{ flex: 1, height: 4, background: '#27272a' }}>
                  <div
                    style={{
                      ...
                      width: `${(scores[name] / maxScore) * 100}%`,
                      transition: 'width 0.3s'
                    }}
                  />
                </div>
                <span style={{ ...mono, fontSize: 12, fontWeight: 700, color: '#fafafa', width: 24, textAlign: 'right' }}>
                  {scores[name]}
                </span>
              </div>
            );
