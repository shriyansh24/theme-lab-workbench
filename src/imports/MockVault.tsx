...
export function MockVault({ c, r = 0 }: { c: ThemeColors; r?: number }) {
  ...
  return (
    <div style={{ ...sans, padding: 16, background: c.bg, minHeight: "100%" }}>
      <PageHeader c={c} r={r} eyebrow="Prepare" title="Document Vault"
        description="Store, preview, and manage resumes and cover letters." />
      <MetricStrip>
      ...
      <SplitWorkspace>
        ...
        <div style={{ flex: 2, display: "flex", flexDirection: "column", gap: 12 }}>
          {/* Upload zone */}
          <div style={{
            border: `2px dashed ${c.border}`, borderRadius: r, padding: 24,
            textAlign: "center", background: c.bgTer,
          }}>
            ...
            {FILES.map(f => (
              <div key={f.name} style={cardS(c, r)}>
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
                  ...
                  <button style={{
                    ...btnS(c, r), padding: "3px 8px", fontSize: 9,
                    color: c.dan, borderColor: c.dan,
                  }}>{"\u2715"}</button>
                </div>
              </div>
            ))}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
          <StateBlock c={c} r={r} title="3 resumes stored"
            description="Latest: resume_v3.pdf, uploaded 2h ago." />
        </div>
      </SplitWorkspace>
    </div>
  );
