...
import { mono, display, cardS, btnS, Badge, PageHeader, MetricCard, MetricStrip, SectionHeader, StateBlock, SplitWorkspace } from "./shared";
...
export function MockDashboard({ c, r = 0 }: { c: ThemeColors; r?: number }) {
  return (
    <div style={{ padding: 16, overflow: "hidden" }}>
      <PageHeader c={c} r={r} eyebrow="Home" title="Command Center" description="Job feed, pipeline, and next actions."
        badges={[
      ...
      <SplitWorkspace>
        <div style={{ flex: 1.25 }}>
          ...
          <div style={{ ...cardS(c, r), padding: 14 }}>
            <SectionHeader c={c} title="Feed" description="Recent jobs" badge={<Badge bg={c.suc} color={textOn(c.suc)} r={r}>Live</Badge>} />
            {JOBS.map((job, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 0", borderBottom: i < JOBS.length - 1 ? `1px solid ${c.bgTer}` : "none" }}>
