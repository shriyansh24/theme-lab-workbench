...
import { PageHeader, MetricStrip, MetricCard, SectionHeader, Badge, StateBlock, Surface, cardS, mono, display, sans, btnS } from "./shared";
...
export function MockNetworking({ c, r = 0 }: { c: ThemeColors; r?: number }) {
  ...
  return (
    <div style={{ ...sans, padding: 16, background: c.bg, minHeight: "100%" }}>
      <PageHeader c={c} r={r} eyebrow="Execute" title="Networking" description="Track contacts, build referral queues, and generate outreach."
        badges={[
      ...
      <div style={{ display: "grid", gridTemplateColumns: "0.95fr 1.05fr", gap: 12 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <SectionHeader c={c} title="Contacts" action={<button style={{ ...btnS(c, r, true), padding: "4px 12px", fontSize: 10 }}>New</button>} />
          <div style={{ border: `2px solid ${c.border}`, borderRadius: r > 0 ? r * 0.5 : 0, padding: "6px 10px", fontSize: 11, color: c.textMut, background: c.bgEl }}>\u2315 Search contacts...</div>
          <div style={{ border: `2px solid ${c.border}`, borderRadius: r, background: c.bgEl, overflow: "hidden", maxHeight: 340 }}>
            {CONTACTS.map((ct, i) => (
              <div key={ct.name} style={{ padding: "8px 10px", borderBottom: i < CONTACTS.length - 1 ? `1px solid ${c.border}` : "none", background: ct.active ? c.bgHov : "transparent", borderLeft: ct.active ? `3px solid ${c.pri}` : "3px solid transparent", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <SectionHeader c={c} title="Editor" action={<button style={{ ...btnS(c, r), padding: "4px 10px", fontSize: 10, color: c.dan, borderColor: c.dan }}>Delete</button>} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {FIELDS.map(f => (
                <div key={f.label}>
                  ...
                  {f.label === "Strength" ? <select style={input(f.val)}><option>{f.val}</option></select> : <div style={input(f.val)}>{f.val}</div>}
                </div>
              ))}
            </div>
            <div style={{ marginTop: 8 }}>
              <div style={{ ...mono, fontSize: 9, color: c.textMut, textTransform: "uppercase", marginBottom: 2 }}>Notes</div>
              <textarea style={{ ...input(""), minHeight: 50, resize: "vertical" }} defaultValue="Met at React Conf 2025. Interested in referral for senior role." />
            </div>
