...
import {
  ...
  Surface, btnS, mono, display, sans,
} from "./shared";
...
export function MockCompanies({ c, r = 0 }: { c: ThemeColors; r?: number }) {
  return (
    <div style={{ ...sans, flex: 1, padding: 16, overflow: "hidden" }}>
      <PageHeader c={c} r={r} eyebrow="Operations" title="Companies"
        description="Canonical company records, ATS fingerprints, and validation confidence."
        badges={[
        ...
        actions={FILTERS.map((f, i) => (
          <button key={f} style={{
            ...btnS(c, r), padding: "6px 12px", fontSize: 11,
      ...
      <Surface c={c} r={r} style={{ overflow: "hidden" }}>
        <div style={{ padding: "10px 12px" }}>
          <SectionHeader c={c} title="Company Registry"
            description="Validation state, ATS routing, and confidence."
            badge={<span style={{ ...mono, fontSize: 10, color: c.textMut }}>1,247 shown</span>}
          />
        </div>
        ...
        <div style={{ width: "100%", borderCollapse: "collapse" } as React.CSSProperties}>
          {/* Header */}
          <div style={{
            display: "grid", gridTemplateColumns: "2fr 1.2fr 1fr 1fr 0.6fr 0.8fr",
            background: c.bgTer, borderBottom: `2px solid ${c.border}`, padding: "8px 12px",
          }}>
            {COLS.map((col, i) => (
              <span key={col} style={{
                ...mono, fontSize: 10, fontWeight: 600, color: c.textMut,
          ...
          {ROWS.map((row, i) => (
            <div key={row.name} style={{
              display: "grid", gridTemplateColumns: "2fr 1.2fr 1fr 1fr 0.6fr 0.8fr",
              padding: "8px 12px", borderBottom: `1px solid ${c.bgTer}`,
              background: i % 2 === 0 ? "transparent" : `${c.bgTer}44`,
              cursor: "pointer",
            }}>
