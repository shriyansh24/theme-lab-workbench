...
export function MockSettings({ c, r = 0 }: { c: ThemeColors; r?: number }) {
  const tabBtnS = (active: boolean) => ({
    ...
    background: active ? c.pri : "transparent", color: active ? textOn(c.pri) : c.textSec,
  ...
  const optBtnS = (active: boolean) => ({
    ...
    background: active ? c.pri : c.bgEl, color: active ? textOn(c.pri) : c.textSec,
    borderRadius: r > 0 ? r * 0.5 : 0,
    display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
  });
  return (
    <div style={{ ...sans, padding: 16, overflow: "hidden" }}>
      <PageHeader c={c} r={r} eyebrow="Operations" title="Settings" description="Account, theme, integrations, and saved searches."
        actions={[<button key="s" style={{ ...btnS(c, r, true), padding: "6px 14px", fontSize: 11, ...mono }}>Save changes</button>]} />
      <div style={{ display: "flex", gap: 16 }}>
        <div style={{ width: 160, flexShrink: 0 }}>
          {TABS.map(tab => <button key={tab} style={tabBtnS(tab === ACTIVE_TAB)}>{tab}</button>)}
        </div>
        <div style={{ flex: 1 }}>
          <SectionHeader c={c} title="Appearance" description="Choose mode and theme family." />
          <div style={{ marginBottom: 16 }}>
            <div style={{ ...display, fontSize: 13, fontWeight: 700, color: c.text, textTransform: "uppercase", marginBottom: 8 }}>Mode</div>
          ...
          <div>
            <div style={{ ...display, fontSize: 13, fontWeight: 700, color: c.text, textTransform: "uppercase", marginBottom: 8 }}>Theme Family</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {FAMILIES.map((f, i) => (
